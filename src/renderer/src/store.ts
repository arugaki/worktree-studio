...
/** 按设置里选的 shell 解析可执行路径(pwsh 用主进程解析到的真实路径) */
function resolveShellPath(pwshPath: string): string {
  switch (getSettings().shell) {
    case 'powershell':
...
interface StoreState {
  ...
  addRootTerminal: (wsId: string) => void
...
export const useStore = create<StoreState>((set, get) => ({
  ...
  addTerminal: (wsId, repo, opts) => {
    ...
    const meta: TerminalMeta = {
      ...
      shellPath: resolveShellPath(get().defaultShell)
    }
    set((s) => ({
  ...
  addRootTerminal: (wsId) => {
    const ws = get().workspaces.find((w) => w.id === wsId)
    if (!ws) return
    // 优先根仓库的 worktree;没有根仓库则落在 worktree 根目录
    const root = ws.repos.find((r) => r.isRoot)
    if (root) get().addTerminal(wsId, root.name)
    else get().addTerminal(wsId, '根目录', { cwd: ws.worktreeRoot })
  },
